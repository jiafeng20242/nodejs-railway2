...
// 修复：下载ZIP包并解压提取二进制
async function downloadAndExtractXray(maxRetries = 3) {
  const arch = getArch();
  const osType = "linux";
  const zipFilename = `Xray-${osType}-${arch}.zip`;
  const zipUrl = `https://github.com/XTLS/Xray-core/releases/latest/download/${zipFilename}`;
  const zipPath = path.join(CONFIG.FILE_PATH, zipFilename);
  const binaryPath = path.join(CONFIG.FILE_PATH, "xray");

  let lastError;

  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      logger.info(`Downloading Xray ZIP (attempt ${attempt}/${maxRetries})...`);
      ...
      // 解压ZIP包
      logger.info("Extracting Xray binary...");
      const zip = fs.createReadStream(zipPath).pipe(unzipper.Parse({ forceStream: true }));
      for await (const entry of zip) {
        if (entry.path === "xray" && entry.type === "File") {
          await new Promise((resolve, reject) => {
            entry.pipe(fs.createWriteStream(binaryPath))
              .on("finish", resolve)
              .on("error", reject);
          });
          break;
  ...
  throw new Error(
    `Failed to download Xray after ${maxRetries} attempts: ${lastError.message}`
  );
}
...
async function boot() {
  ...
  try {
    ...
    const [xrayPath, argoPath] = await Promise.all([
      downloadAndExtractXray(), // 修复：使用新的ZIP下载解压函数
